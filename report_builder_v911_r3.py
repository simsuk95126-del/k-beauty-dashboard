...
REPORT_BUILD_ID = "v9.1.1-report-r3"
...
ENGLISH_CANONICAL_EXACT.update({
    ...
    # One-time status interpretation table
    "판정 용어 해석": "Screening Status Interpretation",
    "짧은 설명": "Quick Meaning",
    "정확한 의미": "Detailed Interpretation",
    "DB에서 위험 항목을 찾지 못함": "No matching risk item was found in the loaded database.",
    "사용 불가 항목과 일치": "Matched an item that cannot be used when the rule applies.",
    "조건을 충족하면 사용할 수 있음": "May be used only when all applicable conditions are met.",
    "표시·고지 요건을 반영해야 함": "Required labeling, warning, disclosure, or notification measures must be implemented.",
    "해당 규정이 이 제품에 적용되는지 확인": "Verify whether the rule applies to this product.",
    "규정 자료를 사람이 확인해야 함": "A person must review the regulatory source and conditions.",
    "성분의 정확한 정체를 먼저 확인해야 함": "Confirm the exact ingredient identity before completing the screening.",
    "현재 성분명 기준 위험 후보가 확인되지 않음": "No HALAL risk indicator was identified from the current ingredient name.",
    "현재 로드된 해당 시장의 규제 DB에서 일치하는 금지·제한·표시·추가확인 항목이 발견되지 않았습니다. 성분 변경이 필요하다는 근거는 없지만, 최종 규제 적합이나 판매 승인을 뜻하지는 않습니다.": "No matching prohibited, restricted, labeling, or additional-verification item was identified in the regulatory database currently loaded for this market. There is no database-based reason to require an ingredient change, but this does not mean final regulatory compliance or sales authorization.",
    "해당 성분이 확정 규제 DB의 금지 항목과 일치했습니다. 동일물질 여부, 적용 범위와 예외를 확인해야 하며, 적용이 맞다면 성분을 제거하거나 대체하기 전에는 현재 처방으로 진행할 수 없습니다.": "The ingredient matched a prohibited item in the confirmed regulatory database. Verify substance identity, scope, and any exemptions. If the rule applies, the current formula cannot proceed until the ingredient is removed or replaced.",
    "해당 성분은 사용할 수 있지만 농도, 제품 유형, 사용 부위, 사용 대상 등의 조건이 붙습니다. 실제 처방이 조건을 충족하는지 확인하기 전에는 통과로 볼 수 없습니다.": "The ingredient may be used only subject to conditions such as concentration, product type, application area, or target user. It must not be treated as passed until the actual formula is shown to meet those conditions.",
    "성분 사용 자체보다 라벨 문구, 경고, 고지, 신고 또는 통지 요건이 문제입니다. 처방 변경이 필요하지 않을 수 있지만 표시·행정 요건은 반드시 반영해야 합니다.": "The main issue is a labeling, warning, disclosure, filing, or notification requirement rather than the use of the ingredient itself. A formula change may not be necessary, but the labeling and administrative requirements must be implemented.",
    "관련 규정 항목은 있지만, 그 규정이 실제 제품 유형·용도·표시광고에 적용되는지는 아직 확정되지 않았습니다. 금지나 제한 판정이 아니라 적용 범위를 먼저 판단해야 하는 상태입니다.": "A relevant regulatory item exists, but whether it applies to the actual product type, intended use, or claims has not yet been determined. This is not a prohibited or restricted decision; the regulatory scope must be assessed first.",
    "검토대기 규제자료와 일치했거나 자동 판정만으로 확정하기 어렵습니다. 공식 원문, 시행 여부, 사용조건을 사람이 확인해야 합니다. 금지 또는 제한으로 확정된 상태는 아닙니다.": "The ingredient matched pending-review regulatory data or cannot be finalized by automated screening alone. A person must verify the official text, effective status, and conditions of use. It has not been confirmed as prohibited or restricted.",
    "공식 INCI명, CAS 번호 또는 원료 정체가 확정되지 않아 정확한 규제 대조를 완료하지 못했습니다. 규제 위험 판정이 아니라 입력 성분의 신원 확인이 먼저 필요한 상태입니다.": "The regulatory comparison could not be completed because the official INCI name, CAS number, or ingredient identity has not been confirmed. This is not a regulatory-risk decision; the input ingredient identity must be verified first.",
    "현재 성분명과 로드된 자료에서는 HALAL 우려 후보가 발견되지 않았습니다. 원료 기원, 제조공정, 가공보조제, 교차오염, 공급사 인증까지 확인된 것은 아니며 HALAL 인증 완료를 의미하지 않습니다.": "No HALAL concern indicator was identified from the current ingredient name and loaded data. Ingredient origin, manufacturing process, processing aids, cross-contamination, and supplier certification have not necessarily been verified, and this does not mean HALAL certification is complete.",
})
...
def _add_simple_table(
    ...
    for row_index, row in enumerate(rows):
        ...
        # Keep each ingredient row together so an INCI name or explanation is
        # not split across two pages.
        row_tr_pr = added_row._tr.get_or_add_trPr()
        cant_split = OxmlElement("w:cantSplit")
        cant_split.set(qn("w:val"), "true")
        row_tr_pr.append(cant_split)
...
def _market_attention_rows(result_data: dict) -> List[List[str]]:
    ...
    if len(details) > 18 and not rows:
        rows.append(["전체 성분", "Excel 참조", "통과", "상세 성분별 결과는 Excel 규제분석결과 시트를 참조하십시오."])
    return rows
...
def _status_interpretation_rows(include_halal: bool = False) -> List[List[str]]:
    rows: List[List[str]] = [
        ["통과", "DB에서 위험 항목을 찾지 못함", "현재 로드된 해당 시장의 규제 DB에서 일치하는 금지·제한·표시·추가확인 항목이 발견되지 않았습니다. 성분 변경이 필요하다는 근거는 없지만, 최종 규제 적합이나 판매 승인을 뜻하지는 않습니다."],
        ["금지", "사용 불가 항목과 일치", "해당 성분이 확정 규제 DB의 금지 항목과 일치했습니다. 동일물질 여부, 적용 범위와 예외를 확인해야 하며, 적용이 맞다면 성분을 제거하거나 대체하기 전에는 현재 처방으로 진행할 수 없습니다."],
        ["제한", "조건을 충족하면 사용할 수 있음", "해당 성분은 사용할 수 있지만 농도, 제품 유형, 사용 부위, 사용 대상 등의 조건이 붙습니다. 실제 처방이 조건을 충족하는지 확인하기 전에는 통과로 볼 수 없습니다."],
        ["경고·표시 필요", "표시·고지 요건을 반영해야 함", "성분 사용 자체보다 라벨 문구, 경고, 고지, 신고 또는 통지 요건이 문제입니다. 처방 변경이 필요하지 않을 수 있지만 표시·행정 요건은 반드시 반영해야 합니다."],
        ["규제 대상", "해당 규정이 이 제품에 적용되는지 확인", "관련 규정 항목은 있지만, 그 규정이 실제 제품 유형·용도·표시광고에 적용되는지는 아직 확정되지 않았습니다. 금지나 제한 판정이 아니라 적용 범위를 먼저 판단해야 하는 상태입니다."],
        ["수동 검토", "규정 자료를 사람이 확인해야 함", "검토대기 규제자료와 일치했거나 자동 판정만으로 확정하기 어렵습니다. 공식 원문, 시행 여부, 사용조건을 사람이 확인해야 합니다. 금지 또는 제한으로 확정된 상태는 아닙니다."],
        ["명칭 검증 필요", "성분의 정확한 정체를 먼저 확인해야 함", "공식 INCI명, CAS 번호 또는 원료 정체가 확정되지 않아 정확한 규제 대조를 완료하지 못했습니다. 규제 위험 판정이 아니라 입력 성분의 신원 확인이 먼저 필요한 상태입니다."],
    ]
    if include_halal:
        rows.append(["HALAL 위험 후보 미확인", "현재 성분명 기준 위험 후보가 확인되지 않음", "현재 성분명과 로드된 자료에서는 HALAL 우려 후보가 발견되지 않았습니다. 원료 기원, 제조공정, 가공보조제, 교차오염, 공급사 인증까지 확인된 것은 아니며 HALAL 인증 완료를 의미하지 않습니다."])
    return rows
...
def create_product_report_bytes(
    ...
    _section_title(document, "4. 판정 용어 해석")
    _add_simple_table(
        document,
        ["판정", "짧은 설명", "정확한 의미"],
        _status_interpretation_rows(include_halal=halal_result is not None),
        NAVY,
        7.7,
        0,
    )
    ...
    for target in selected_markets:
        ...
        if action_rows:
            _add_simple_table(
                document,
                ["입력 성분명", "INCI Name", "판정", "해야 할 일", "필요한 자료", "완료 기준"],
                action_rows,
                BLUE,
                7.7,
                2,
        ...
        if halal_result is not None:
            ...
            if link_rows:
                _add_simple_table(
                    document,
                    ["HALAL 확인 성분", "INCI Name", "검토 상태", "시장에 미치는 영향", "필요한 자료"],
                    link_rows,
                    NAVY,
                    7.8,
                    2,
        ...
        if manual:
            ...
            _add_simple_table(
                document,
                ["입력 성분명", "INCI Name", "현재 판정", "확인 이유", "필요한 조치"],
                manual,
                BLUE,
                7.9,
                2,
    ...
    if halal_result is not None:
        ...
        _add_simple_table(
            document,
            ["입력 성분명", "INCI Name", "검토 결과", "확인 이유", "필요한 자료"],
            rows,
            NAVY,
            7.9,
            2,
        ...
        if manual:
            ...
            _add_simple_table(
                document,
                ["입력 성분명", "INCI Name", "확인 유형", "확인할 사항", "필요한 자료"],
                manual,
                BLUE,
                7.9,
                2,
